...
enum ENEMYSTATE { MOVE, DIE, ATTACK }
...
func _process(delta):
	if state == ENEMYSTATE.MOVE:
		var direction := (
			(Vector3(randi() % 3 - 1, randi() % 3 - 1, randi() % 3 - 1) + last_direction).normalized()
			if randi() % 2 == 0
			else Vector3()
		)
...
func _on_EntityHitBox_area_entered(area: Area):
	if area is EntityHurtBox:
		...
		var direction = (
			(area.global_transform.origin - global_transform.origin).normalized()
			* collision_impulse
		)
